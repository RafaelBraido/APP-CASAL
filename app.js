...
// ===== Modo somente front (hospedagem estática, ex.: Hostinger) =====
// Sem servidor, o site funciona 100% no navegador: os dados são
// salvos localmente (localStorage), apenas neste dispositivo.
const LS_DEV = "labiak_local_devocionais";
const LS_PREG = "labiak_local_pregacoes";
const LS_USERS = "labiak_local_usuarios";
const LS_RESULTS = "labiak_local_resultados";
const LS = {
  get(k, fb) { try { return JSON.parse(localStorage.getItem(k)) ?? fb; } catch { return fb; } },
  set(k, v) { localStorage.setItem(k, JSON.stringify(v)); },
};
let semBackend = false;
...
async function apiFetch(url, opcoes = {}) {
  ...
  if (resposta.status === 404) semBackend = true;
...
document.getElementById("formLogin").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!semBackend) { erro.textContent = err.message; return; }
    const nome = document.getElementById("loginNome").value.trim();
    if (!nome) { erro.textContent = "Informe seu nome."; return; }
    localStorage.setItem(USER_TOKEN_KEY, "local");
...
document.getElementById("formCadastro").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!nome) { erro.textContent = "Informe seu nome."; return; }
    const usuarios = LS.get(LS_USERS, []);
    usuarios.push({ nome, email: document.getElementById("cadastroEmail").value.trim(), telefone: document.getElementById("cadastroTelefone").value.trim(), role: "user", createdAt: new Date().toISOString() });
    LS.set(LS_USERS, usuarios);
    localStorage.setItem(USER_TOKEN_KEY, "local");
    localStorage.setItem(USER_NAME_KEY, nome);
    document.getElementById("formCadastro").reset();
    fecharUser();
...
async function carregarDevocionais() {
  ...
  try {
    ...
    todosDevocionais = LS.get(LS_DEV, []);
  }
  renderDevocionais();
}
...
// ===== Pregações =====
function renderPregacoesLista(lista) {
...
async function carregarPregacoes() {
  ...
  try {
    renderPregacoesLista(await apiFetch("/pregacao"));
  } catch (e) {
    renderPregacoesLista(LS.get(LS_PREG, []));
  }
}
...
document.getElementById("formAdminLogin").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!semBackend) { erro.textContent = err.message; return; }
    if (document.getElementById("adminCodigo").value !== "1234") { erro.textContent = "Código inválido."; return; }
    localStorage.setItem(ADMIN_TOKEN_KEY, "local");
    document.getElementById("adminCodigo").value = "";
    mostrarPainel();
    toast("Bem-vindo, pastor(a)!");
  }
...
document.getElementById("formDevocional").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!semBackend) { erro.textContent = err.message; return; }
    const devocionais = LS.get(LS_DEV, []);
    devocionais.unshift({
      _id: "l" + Date.now(),
      titulo: document.getElementById("devTitulo").value.trim(),
      versiculo: document.getElementById("devVersiculo").value.trim(),
...
document.getElementById("formPregacao").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!semBackend) { erro.textContent = err.message; return; }
    const url = document.getElementById("pregUrl").value.trim();
    const m = url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/|v\/)|youtu\.be\/)([\w-]{11})/);
    if (!m) { erro.textContent = "URL do YouTube inválida."; return; }
    const pregacoes = LS.get(LS_PREG, []);
    pregacoes.unshift({
      _id: "l" + Date.now(),
      titulo: document.getElementById("pregTitulo").value.trim(),
      descricao: document.getElementById("pregDescricao").value.trim(),
...
async function carregarAdminDevocionais() {
  ...
  let lista;
  try {
    lista = await apiFetch("/devocional");
  } catch (e) {
    lista = LS.get(LS_DEV, []);
  }
...
async function carregarAdminPregacoes() {
  ...
  let lista;
  try {
    lista = await apiFetch("/pregacao");
  } catch (e) {
    lista = LS.get(LS_PREG, []);
  }
...
function renderAdminUsers(stats, usuarios) {
...
async function carregarAdminUsers() {
  try {
    ...
    renderAdminUsers(stats, usuarios);
  } catch (e) {
    if (!semBackend) { toast("Erro ao carregar usuários: " + e.message); return; }
    const usuarios = LS.get(LS_USERS, []);
    const testesPorTipo = {};
    LS.get(LS_RESULTS, []).forEach((r) => { testesPorTipo[r.tipoTeste] = (testesPorTipo[r.tipoTeste] || 0) + 1; });
    renderAdminUsers({
      totalUsuarios: usuarios.length,
      totalAdmins: 0,
      totalTutoriais: LS.get(LS_DEV, []).length,
      totalResultados: LS.get(LS_RESULTS, []).length,
      testesPorTipo,
    }, usuarios);
  }
}
...
async function carregarAdminResults() {
  ...
  let lista;
  try {
    lista = await apiFetch("/Admin/resultados");
  } catch (e) {
    lista = LS.get(LS_RESULTS, []);
  }
...
document.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (!semBackend) { toast(err.message); return; }
    const key = tipo === "dev" ? LS_DEV : LS_PREG;
    LS.set(key, LS.get(key, []).filter((x) => x._id !== id));
  }
