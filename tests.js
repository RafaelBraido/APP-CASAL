...
function finalizar(container) {
  ...
  if (logado) {
    apiFetch("/resultado", {
      ...
      // Modo somente front: salvar o resultado no navegador
      const resultados = LS.get(LS_RESULTS, []);
      resultados.unshift({ tipoTeste: teste.id, titulo: r.titulo, resumo: r.resumo, detalhes: r.detalhes, usuario: { nome: localStorage.getItem(USER_NAME_KEY) || "—" }, createdAt: new Date().toISOString() });
      LS.set(LS_RESULTS, resultados);
      const saved = document.getElementById("resultSaved");
      if (saved) saved.hidden = false;
      toast("Resultado salvo!");
    });
